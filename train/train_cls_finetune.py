...
def main(cfg_path="configs/cls_pred_ft.yaml"):
    cfg = yaml.safe_load(open(cfg_path, "r"))
    device = "cuda" if torch.cuda.is_available() else "cpu"

    run_name = time.strftime("cls_pred_ft_%Y%m%d-%H%M%S")
    log_dir = os.path.join("runs", "cls_pred_ft", "tb", run_name)
    ...
    # Transforms (add slight crop jitter to mimic bbox noise)
    train_tf = A.Compose([
        A.Resize(cfg["img_size"] + 64, cfg["img_size"] + 64),
    ...
    # Build model (same backbone)
    model = timm.create_model(cfg["backbone"], pretrained=False, num_classes=len(stages))

    # Load init checkpoint (fine-tune start)
    init_ckpt = torch.load(cfg["init_checkpoint"], map_location="cpu")
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for x, y in tqdm(train_loader, desc=f"ClsPredFT Train {epoch+1}/{cfg['epochs']}"):
        ...
        writer.add_scalar("cls_pred_ft/train_loss", avg_train_loss, epoch + 1)
        writer.add_scalar("cls_pred_ft/val_loss", avg_val_loss, epoch + 1)
        writer.add_scalar("cls_pred_ft/val_macro_f1", macro_f1, epoch + 1)
        writer.add_scalar("cls_pred_ft/lr", lr, epoch + 1)

        metrics.append({"epoch": epoch + 1, "train_loss": avg_train_loss, "val_loss": avg_val_loss, "val_macro_f1": macro_f1, "lr": lr})
        ...
        if macro_f1 > best_f1:
            ...
            print("Saved best cls_pred_ft checkpoint.")
